...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro === true) {
    next();
  } else if (numberTodos < 10) {
    next();
  } else {
    response.status(403).json({
      error: "Assine o plano Pro para melhorar sua organização de tarefas",
    });
  }
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!user) {
    ...
    const newTodo = user.todos.find((todo) => {
      return todo.id === id;
    });
    if (!newTodo) {
      return response
        .status(404)
        .json({ error: "This todo does not belong to this user!" });
    } else {
      request.user = user;
